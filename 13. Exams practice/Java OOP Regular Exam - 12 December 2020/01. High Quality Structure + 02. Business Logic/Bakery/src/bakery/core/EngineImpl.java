...
public class EngineImpl implements Engine {
  ...
  private String processInput() throws IOException {
    ...
    String result = switch (command) {
      case AddDrink -> this.controller.addDrink(
              data[0], data[1], Integer.parseInt(data[2]), data[3]
      );
      case AddFood -> this.controller.addFood(
      ...
      case ReserveTable -> this.controller.reserveTable(Integer.parseInt(data[0]));
      case OrderDrink -> this.controller.orderDrink(Integer.parseInt(data[0]), data[1], data[2]);
      case OrderFood -> this.controller.orderFood(Integer.parseInt(data[0]), data[1]);
      case LeaveTable -> this.controller.leaveTable(Integer.parseInt(data[0]));
      case GetFreeTablesInfo -> this.controller.getFreeTablesInfo();
      case GetTotalIncome -> this.controller.getTotalIncome();
