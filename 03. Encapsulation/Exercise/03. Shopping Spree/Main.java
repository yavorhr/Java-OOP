...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        List<Person> people = Arrays.stream(scanner.nextLine().split(";")).map(personInfo -> {
        ...
        List<Product> products = Arrays.stream(scanner.nextLine().split(";")).map(productInfo -> {
            ...
            for (Person p : people) {
                if (p.getName().equals(personName)) {
                    try {
                        ...
                        System.out.println(personName + " bought " + product);
